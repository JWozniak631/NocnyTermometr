...
def request_open_meteo(params: dict[str, Any], max_attempts: int = 4) -> Any:
    """Pobiera JSON z prostym backoffem.

    Streamlit Community Cloud współdzieli adresy IP między wieloma aplikacjami, więc
    sporadyczne HTTP 429 może pojawić się nawet przy sensownym kodzie. Zamiast
    natychmiast wywalić aplikację, robimy krótkie retry i szanujemy Retry-After,
    jeśli API go poda.
    """
    headers = {
        "User-Agent": "nocny-termometr-polski-streamlit/1.1 (student project)",
        "Accept": "application/json",
    }
    last_exc: Exception | None = None
    for attempt in range(max_attempts):
        try:
            response = requests.get(API_URL, params=params, timeout=45, headers=headers)
            if response.status_code == 429 and attempt < max_attempts - 1:
                retry_after = response.headers.get("Retry-After")
                wait_seconds = int(retry_after) if retry_after and retry_after.isdigit() else 8 * (attempt + 1)
                time.sleep(min(wait_seconds, 35))
                continue
    ...
    if last_exc is not None:
        raise last_exc
    raise RuntimeError("Nieznany błąd pobierania danych z Open-Meteo.")


@st.cache_data(ttl=7 * 24 * 3600, show_spinner=False)
def fetch_weather_bulk(cities: Tuple[str, ...], start: str, end: str) -> pd.DataFrame:
    """Pobiera wiele miast jednym zapytaniem HTTP.

    Pierwsza wersja projektu robiła osobny request dla każdego miasta i zawsze
    pobierała pełny zakres 2019-2024. Na wdrożeniu mogło to szybko kończyć się
    błędem 429. Ta wersja pobiera tylko miasta i daty wybrane w panelu bocznym.
    """
    selected = [city for city in cities if city in CITY_META]
    if not selected:
        raise ValueError("Nie wybrano poprawnych miast do pobrania.")

    params = {
        "latitude": ",".join(f"{CITY_META[city]['lat']:.4f}" for city in selected),
        "longitude": ",".join(f"{CITY_META[city]['lon']:.4f}" for city in selected),
        "start_date": start,
        "end_date": end,
    ...
    payload = request_open_meteo(params)
    payloads = payload if isinstance(payload, list) else [payload]
    if len(payloads) != len(selected):
        # Zapasowy parser: gdy API zwróci jeden obiekt zamiast listy, nadal obsłużymy 1 miasto.
        if len(selected) == 1 and len(payloads) == 1:
            pass
        else:
            raise ValueError(
                f"API zwróciło {len(payloads)} serii, a oczekiwano {len(selected)}. "
                "Spróbuj zmniejszyć liczbę miast lub zakres dat."
            )

    frames: list[pd.DataFrame] = []
    for city, item in zip(selected, payloads):
        daily = item.get("daily", {}) if isinstance(item, dict) else {}
        if not daily or "time" not in daily:
            raise ValueError(f"API nie zwróciło dziennych danych dla miasta: {city}")
        frame = pd.DataFrame(daily)
        frame["city"] = city
        frame["lat"] = CITY_META[city]["lat"]
        frame["lon"] = CITY_META[city]["lon"]
        frames.append(frame)

    return pd.concat(frames, ignore_index=True)


@st.cache_data(ttl=7 * 24 * 3600, show_spinner="Pobieram i czyszczę dane pogodowe…")
def load_raw_data(cities: Tuple[str, ...], start: str, end: str) -> pd.DataFrame:
    return fetch_weather_bulk(cities, start, end)
...
city_choice = st.sidebar.multiselect(
    "Miasta",
    options=all_cities,
    default=["Warszawa", "Kraków", "Wrocław", "Gdańsk"],
)
if not city_choice:
...
if isinstance(selected_range, tuple) and len(selected_range) == 2:
    ...
    date_from, date_to = date(2023, 1, 1), DATA_END
...
# ------------------------------
try:
    raw_weather = load_raw_data(tuple(city_choice), date_from.isoformat(), date_to.isoformat())
    weather, cleaning = prepare_data(raw_weather)
except requests.HTTPError as exc:
    status = getattr(exc.response, "status_code", None)
    if status == 429:
        st.error(
            "Open-Meteo chwilowo odrzuciło zapytanie: HTTP 429 Too Many Requests. "
            "Odczekaj kilka minut, zmniejsz zakres dat/miast albo kliknij ponownie po chwili. "
            "Aplikacja ma już cache, retry i pobieranie wielu miast jednym requestem, więc po pierwszym udanym pobraniu "
            "nie powinna ponownie uderzać w API przy każdej zmianie filtrów."
        )
    else:
    ...
    st.error("Nie udało się przygotować danych. Spróbuj zmniejszyć zakres dat/miast albo uruchomić aplikację ponownie.")
    with st.expander("Szczegóły techniczne"):
        st.exception(exc)
    st.stop()
